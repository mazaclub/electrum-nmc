...
class InstallWizard(QDialog):
    ...
    def __init__(self, config, network, storage):
        ...
        self.setWindowTitle('Electrum-NMC' + '  -  ' + os.path.basename(self.storage.path))
    ...
    def network_dialog(self):
        ...
        label = QLabel(_("Electrum-NMC communicates with remote servers to get information about your transactions and addresses. The servers all fulfil the same purpose only differing in hardware. In most cases you simply want to let Electrum-NMC pick one at random if you have a preference though feel free to select a server manually.") + "\n\n" \
    ...
    def run(self, action):
        ...
        if action == 'new':
            ...
            if wallet_type == 'multisig':
                ...
                hardware_wallets = map(lambda x:(x[1],x[2]), filter(lambda x:x[0]=='hardware', electrum_nmc.wallet.wallet_types))
